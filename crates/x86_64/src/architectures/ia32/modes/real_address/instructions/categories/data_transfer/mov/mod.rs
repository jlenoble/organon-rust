...
pub trait Mov<Dest, Src> {
    ...
    fn mov(&self, dest: Dest, src: Src) -> Vec<u8>;
}
...
macro_rules! r8_imm8 {
    ($r:ty, $op_code:expr) => {
        impl Mov<$r, Imm8> for ASM {
            #[inline]
            fn mov(&self, _: $r, imm: Imm8) -> Vec<u8> {
                vec![$op_code, imm.0]
            }
        }
    };
...
macro_rules! r16_imm16 {
    ($r:ty, $op_code:expr) => {
        impl Mov<$r, Imm16> for ASM {
            #[inline]
            fn mov(&self, _: $r, imm: Imm16) -> Vec<u8> {
...
macro_rules! r_r {
    ($r1:tt, $r2:tt, $op_code:literal) => {
        impl Mov<$r1, $r2> for ASM {
            #[inline]
            fn mov(&self, _r1: $r1, _r2: $r2)-> Vec<u8> {
...
macro_rules! r_mem_num {
    (AL, $op_code:literal) => {
        impl Mov<AL, [u16; 1]> for ASM {
            #[inline]
            fn mov(&self, _: AL, mem: [u16; 1]) -> Vec<u8> {
    ...
    (AX, $op_code:literal) => {
        impl Mov<AX, [u16; 1]> for ASM {
            #[inline]
            fn mov(&self, _: AX, mem: [u16; 1]) -> Vec<u8> {
                vec![$op_code, (mem[0] & 0xff) as u8, ((mem[0] & 0xff00) >> 8) as u8]
            }
        }
    };
    ($r:tt, $op_code:literal) => {
        impl Mov<$r, [u16; 1]> for ASM {
            #[inline]
            fn mov(&self, _: $r, mem: [u16; 1]) -> Vec<u8> {
...
macro_rules! r_mem_reg {
    ($r1:tt, BP, $op_code:literal) => {
        impl Mov<$r1, [BP; 1]> for ASM {
            #[inline]
            fn mov(&self, _: $r1, _mem: [BP; 1]) -> Vec<u8> {
    ...
    ($r1:tt, $r2:tt, $op_code:literal) => {
        ...
        #[inline]
            fn mov(&self, _: $r1, _mem: [$r2; 1]) -> Vec<u8> {
...
macro_rules! rr8_mem8_reg {
    ($r2:tt, $op_code:literal) => {
        r_mem_reg!(AL, $r2, $op_code);
        r_mem_reg!(CL, $r2, $op_code);
        r_mem_reg!(DL, $r2, $op_code);
        r_mem_reg!(BL, $r2, $op_code);
        r_mem_reg!(AH, $r2, $op_code);
        r_mem_reg!(CH, $r2, $op_code);
        r_mem_reg!(DH, $r2, $op_code);
        r_mem_reg!(BH, $r2, $op_code);
    };
}

// *ref.: Intel® 64 and IA-32 Architectures Software Developer’s Manual, Vol. 2, Section 4.3#MOV line `8A /r`*
rr8_mem8_reg!(BX, 0x8a);
rr8_mem8_reg!(SI, 0x8a);
rr8_mem8_reg!(DI, 0x8a);

// *ref.: Intel® 64 and IA-32 Architectures Software Developer’s Manual, Vol. 2, Section 4.3#MOV line `8A /r`*
rr8_mem8_reg!(BP, 0x8a);

macro_rules! rr16_mem16_reg {
    ($r2:tt, $op_code:literal) => {
        r_mem_reg!(AX, $r2, $op_code);
        r_mem_reg!(CX, $r2, $op_code);
        r_mem_reg!(DX, $r2, $op_code);
        r_mem_reg!(BX, $r2, $op_code);
        r_mem_reg!(SP, $r2, $op_code);
        r_mem_reg!(BP, $r2, $op_code);
        r_mem_reg!(SI, $r2, $op_code);
        r_mem_reg!(DI, $r2, $op_code);
    };
}
